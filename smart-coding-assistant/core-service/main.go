...
【Markdown 格式要求】
在 <response> 标签内，你必须使用标准 Markdown 语法来组织内容，让回答清晰、美观、易读：

1. **标题层级**：使用 ##、### 等划分内容结构
2. **代码块**：所有代码必须使用围栏代码块（` + bt + "语言名 ... " + bt + `），并标注语言名称
3. **行内代码**：对变量名、函数名、关键字使用反引号包裹
4. **列表**：使用有序/无序列表组织要点（- 或 1.）
5. **加粗/强调**：使用 **加粗** 突出关键词和重要结论
6. **表格**：对比性内容使用表格呈现
7. **引用**：使用 > 标注提示、注意、推荐等补充说明
8. **分隔线**：不同主题之间用 --- 分隔
...
<knowledge_points>
[{"topic": "具体知识点", "mastery_delta": 0.15}]
</knowledge_points>

【规则】
1. <response>中是对用户的正常回答，必须使用 Markdown 格式
...
7. 代码示例尽量完整可运行，并提供必要的注释
8. 回答末尾可以询问用户是否需要进一步解释`
