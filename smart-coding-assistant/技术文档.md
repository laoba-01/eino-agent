...
- **HTTP 接口**：
  ...
  - `/api/learning/report`：获取学习报告（需要认证）
  - `/api/learning/points`：获取知识点列表（需要认证）
  - `/health`：健康检查
...
- **gRPC 接口**：
  ...
  - `SaveKnowledgePoint`：保存知识点（Redis）
  - `GetKnowledgePoints`：获取知识点列表（Redis）
  - `GetLearningStats`：获取学习统计（Redis）
- **功能**：
  - 与 Redis 交互，存储学习上下文
